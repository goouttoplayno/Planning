...
import com.example.nationstyle.TextureUtils;
...
public class FlipCards {
	...
	public void reloadTexture(int frontIndex, View frontView, int backIndex,
		...
		synchronized (this) {
			...
			if (AphidLog.ENABLE_DEBUG) {
				AphidLog.d(
						"reloadTexture: activeIndex %d, front %d, back %d, angle %.1f",
						getPageIndexFromAngle(accumulatedAngle), frontIndex,
						backIndex, accumulatedAngle);
			}
		}
	}
	...
	public synchronized void draw(FlipRenderer renderer, GL10 gl) {
		...
		if ((frontCards.getView() == null || TextureUtils
				.isValidTexture(frontCards.getTexture()))
				&& (backCards.getView() == null || TextureUtils
	...
	private void setState(int state) {
		if (this.state != state) { /*
									 * if (AphidLog.ENABLE_DEBUG) AphidLog.i(
									 * "setState: from %d, to %d; angle %.1f",
